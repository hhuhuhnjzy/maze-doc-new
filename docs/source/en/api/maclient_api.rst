...
.. py:class:: MaClient(server_url="http://localhost:8000")
   ...
   **Example:**
   ...
   .. py:method:: get_workflow(workflow_id)

      Retrieve an existing workflow by its ID.

      :param str workflow_id: The unique identifier of the workflow
      ...
      **Example:**
      ...
      .. note::
         This method does not verify that the workflow exists on the server. It simply creates a local workflow object with the given ID.

   .. py:method:: get_ray_head_port()

      Get the Ray head node port information for worker connections.

      :returns: A dictionary containing port information
      :rtype: dict
      ...
      **Example:**
      ...
      .. code-block:: python
          ...
          print(f"Ray head port: {port_info}")
...
.. py:class:: MaTask(task_id, workflow_id, server_url, task_name=None, output_keys=None)
   ...
   .. py:method:: save(code_str, task_input, task_output, resources)

      Manually save task configuration (legacy API).

      :param str code_str: The task's Python code as a string
      :param dict task_input: Input parameter configuration
      :param dict task_output: Output parameter configuration
      :param dict resources: Resource requirements
      :raises Exception: If saving fails

      .. deprecated::
         This method is part of the legacy API. When using the ``@task`` decorator with :py:meth:`MaWorkflow.add_task`, task configuration is saved automatically.
...
.. code-block:: python
    ...
    # Define tasks
    @task(inputs=["data"], outputs=["preprocessed"])
    def preprocess(params):
        data = params.get("data")
        return {"preprocessed": data.strip().lower()}

    @task(inputs=["data"], outputs=["analyzed"])
    def analyze(params):
        data = params.get("data")
        return {"analyzed": f"Analysis of: {data}"}

    @task(inputs=["data"], outputs=["exported"])
    def export(params):
        data = params.get("data")
        return {"exported": f"Exported: {data}"}
    ...
    # Add tasks
    t1 = workflow.add_task(preprocess, inputs={"data": "  RAW DATA  "})
    t2 = workflow.add_task(analyze, inputs={"data": t1.outputs["preprocessed"]})
    t3 = workflow.add_task(export, inputs={"data": t2.outputs["analyzed"]})
...
.. code-block:: python
    ...
    def fetch(params):
        return {"data": f"Data from {params.get('source')}"}

    @task(inputs=["data"], outputs=["result"])
    def process_a(params):
        return {"result": f"{params.get('data')} -> A"}

    @task(inputs=["data"], outputs=["result"])
    def process_b(params):
        return {"result": f"{params.get('data')} -> B"}

    @task(inputs=["result_a", "result_b"], outputs=["merged"])
    def merge(params):
        a = params.get("result_a")
        b = params.get("result_b")
    ...
    t1 = workflow.add_task(fetch, inputs={"source": "database"})
    t2 = workflow.add_task(process_a, inputs={"data": t1.outputs["data"]})
    t3 = workflow.add_task(process_b, inputs={"data": t1.outputs["data"]})
    t4 = workflow.add_task(
        merge,
        inputs={
            "result_a": t2.outputs["result"],
            "result_b": t3.outputs["result"]
        }
    )
